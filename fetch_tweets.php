...
if (!$retweeted)
  $shared = 1;
if ($top_images || $top_videos || $top_links)
  $tops = 1;
...
if ($_GET['inspect']) {
  ...
  if (strlen($partial_text) < 20)
    $clear_text = "";

  if ($clear_text) {
    $clear_text = "OR clear_text='" . $link->real_escape_string($clear_text) .
  ...
  if ($debug && $_SESSION[basename(__DIR__) . 'email'] == $admin_email)
    echo "<hr>(" . $query . ")";

  if ($result = $link->query($query)) {
    if (!$result->num_rows)
      die("<b><br>This tweet has no replies or mentions<br></b><a href='#' onclick=javascript:GetDetails('$url')><small>Back to the main list</small></a></center>");
  ...
  while ($row = $result->fetch_assoc()) {
    ...
    if ($row['in_reply_to_tweet'] == $tweet_id) {
      $found = "Direct tweet reply";
      $note = "block";
      $t++;
      $tweet_marker = $tweet_id . "_t";
    }
    elseif ($row['in_reply_to_user'] == $user_id) {
      $found = "General reply to user (excluding direct replies)";
      $note = "success";
      $u++;
      $tweet_marker = $tweet_id . "_u";
    }
    elseif ($clear_text && $row['clear_text'] == $clear_text) {
      $found = "Retweeted tweet";
      $note = "info";
      $r++;
      $tweet_marker = $tweet_id . "_r";
    }
    elseif ($row['quoted_tweet_id'] == $tweet_id) {
      $found = "Quoted tweet";
      $note = "info";
      $q++;
      $tweet_marker = $tweet_id . "_q";
      $is_quote = 1;
    }
    elseif ($clear_text && $row['clear_text'] == $original_text) {
      $found = "Used original tweet";
      $note = "success";
      $o++;
      $tweet_marker = $tweet_id . "_o";
    }
    elseif ($clear_text && strpos($row['clear_text'], $partial_text) == true) {
      $found = "Partial text match";
      $note = "block";
      $p++;
      $tweet_marker = $tweet_id . "_p";
    }
    elseif (strpos(strtolower($row['user_mentions']), strtolower($user_name)) == true) {
      $found = "A mention of @$user_name (excluding direct or user replies)";
      $note = "error";
      $m++;
      $tweet_marker = $tweet_id . "_m";
    }
    $i++;
  ...
  if (!$i)
    die("No related tweets to ($user_name) were found...");
  if (!$_GET['branch']) {
    ...
    echo "<ul>";
    if ($t)
      echo "<li><b>$t</b> direct replies to the tweet</li>";
    if ($u)
      echo "<li><b>$u</b> replies to the tweeter (excludes direct replies) in the next 24 hours</li>";
    if ($m)
      echo "<li><b>$m</b> mentions of @$user_name (excludes direct and tweeter replies) in the next 24 hours</li>";
    if ($r)
      echo "<li><b>$r</b> a manual retweet (not through Twitter's retweet feature)</li>";
    if ($o)
      echo "<li><b>$o</b> exact copies of the tweet</li>";
    if ($p)
      echo "<li><b>$p</b> almost identical matches of the tweet</li>";
    if ($q)
      echo "<li><b>$q</b> quote tweets</li>";
    echo "</ul><b>Find the replies/mentions below sorted by the time they were posted:</b><br><br>";
  }
  echo "</center>";
  if ($t)
    echo "<input type=checkbox id='show_t' name='show_t' checked onclick=toggle_tweets('$tweet_id','t');> Show tweet replies ($t)";
  if ($u)
    echo "&nbsp; <input type=checkbox id='show_u' name='show_u' checked onclick=toggle_tweets('$tweet_id','u');> Show user replies ($u)";
  if ($m)
    echo "&nbsp; <input type=checkbox id='show_m' name='show_m' checked onclick=toggle_tweets('$tweet_id','m');> Show user mentions ($m)";
  if ($r)
    echo "&nbsp; <input type=checkbox id='show_r' name='show_r' checked onclick=toggle_tweets('$tweet_id','r');> Show manual retweets ($r)";
  if ($o)
    echo "&nbsp; <input type=checkbox id='show_o' name='show_o' checked onclick=toggle_tweets('$tweet_id','o');> Show replicas ($o)";
  if ($p)
    echo "&nbsp; <input type=checkbox id='show_p' name='show_p' checked onclick=toggle_tweets('$tweet_id','p');> Show partial matches ($p)";
  if ($q)
    echo "&nbsp; <input type=checkbox id='show_q' name='show_q' checked onclick=toggle_tweets('$tweet_id','q');> Show quoted tweets ($q)";
...
if (!$_GET['point']) {
  ...
  if ($_GET['only_tweets'])
    $condition = $condition . " AND (is_retweet<>1)";
  if ($startdate)
    $from = trim("$startdate $starttime");
  if ($enddate)
    $to = trim("$enddate $endtime");
  if ($from)
    $condition = $condition . " AND date_time>='$from'";
  if ($to)
    $condition = $condition . " AND date_time<'$to'";

  if ($sources == "only_web") {
    $condition = $condition . " AND NOT (source LIKE '%Android%' OR source LIKE '%iPad%' OR source LIKE '%BlackBerry%' OR source LIKE '%Mobile%' OR source LIKE '%Nokia%' OR source LIKE '%Symbian%' OR source LIKE '%Phone%' OR source LIKE '%Tab%' OR source LIKE '%App%') AND (source LIKE '%Web%')";
    $name = $name . " (web sources)";
  }
  elseif ($sources == "only_mobile") {
    $condition = $condition . " AND (source LIKE '%Android%' OR source LIKE '%iPad%' OR source LIKE '%BlackBerry%' OR source LIKE '%Mobile%' OR source LIKE '%Nokia%' OR source LIKE '%Symbian%' OR source LIKE '%Phone%' OR source LIKE '%Tab%' OR source LIKE '%App%') AND (source NOT LIKE '%Web%')";
    $name = $name . " (mobile sources)";
  }
  elseif ($sources) {
    $c = "";
    $started = false;
    ...
    foreach ($tmp as $k) {
      if (!$started)
        $c = "AND ((source is not null and LOWER(source) like '%" . $link->real_escape_string(trim(strtolower($k))) . "%') ";
      else
        $c = $c . " OR (source is not null and LOWER(source) like '%" . $link->real_escape_string(trim(strtolower($k))) . "%') ";
      $started = true;
    }
  ...
  if ($languages == "en")
    $condition = $condition . " AND (tweet_language='en')";
  elseif ($languages)
    $condition = $condition . " AND (tweet_language like '%" . strtolower($languages) . "%')";

  if ($_GET['types'] == "some") {
    if ($_GET['bool_op'] == "NOT" || $_GET['bool_op'] == "  OR ") {
      $bool_op = " AND NOT (";
      $_GET['bool_op'] = "  OR ";
    }
    else
      $bool_op = " AND (";
    if ($_GET['image_tweets']) {
      $condition = $condition . " $bool_op has_image=1 ";
      $name = $name . " (with image only)";
      $bool_op = $_GET['bool_op'];
    }
    if ($_GET['video_tweets']) {
      $condition = $condition . " $bool_op has_video=1 ";
      $name = $name . " (with video only)";
      $bool_op = $_GET['bool_op'];
    }
    if ($_GET['link_tweets']) {
      $condition = $condition . " $bool_op has_link=1 ";
      $name = $name . " (with link only)";
      $bool_op = $_GET['bool_op'];
    }
    if ($_GET['original_tweets']) {
      $condition = $condition . " $bool_op (is_retweet<>1)";
      $name = $name . " (are original tweets)";
      $bool_op = $_GET['bool_op'];
    }
    if ($_GET['retweet_tweets']) {
      $condition = $condition . " $bool_op (is_retweet=1) ";
      $name = $name . " (are retweets)";
      $bool_op = $_GET['bool_op'];
    }
    if ($_GET['response_tweets']) {
      $condition = $condition . " $bool_op (is_reply=1 OR in_reply_to_user is not null)";
      $name = $name . " (are replies)";
      $bool_op = $_GET['bool_op'];
    }
    if ($_GET['referenced_tweets']) {
      $condition = $condition . " $bool_op (is_referenced=1) ";
      $name = $name . " (are referenced tweets)";
      $bool_op = $_GET['bool_op'];
    }
    if ($_GET['quoting_tweets']) {
      $condition = $condition . " $bool_op (is_quote=1) ";
      $name = $name . " (quotes another tweet)";
      $bool_op = $_GET['bool_op'];
    }

    if ($_GET['mentions_tweets']) {
      $condition = $condition . " $bool_op (clear_text like '@%') ";
      $name = $name . " (are responses)";
      $bool_op = $_GET['bool_op'];
    }
    if ($_GET['responded_tweets']) {
      $condition = $condition . " $bool_op (replies is not null AND replies>0) ";
      $name = $name . " (are responsed to)";
      $bool_op = $_GET['bool_op'];
    }
    if ($_GET['exact_phrase']) {
      $condition = $condition . " $bool_op LOWER(clear_text) REGEXP '([[:blank:][:punct:]]|^)" . $link->real_escape_string($_GET['exact_phrase']) . "([[:blank:][:punct:]]|$)'";
      $name = $name . " (exact phrase search)";
      $bool_op = $_GET['bool_op'];
    }
    if ($_GET['user_verified']) {
      $condition = $condition . " $bool_op user_verified=1 ";
      $name = $name . " (from a verified tweeter)";
      $bool_op = $_GET['bool_op'];
    }
    if ($_GET['min_retweets']) {
      $condition = $condition . " $bool_op retweets>=" . $_GET['min_retweets'] . " ";
      $name = $name . " (with minimum # of " . $_GET['min_retweets'] . " retweets)";
      $bool_op = $_GET['bool_op'];
    }
    if ($_GET['any_hashtags']) {
      ...
      foreach ($tmp as $k) {
        if (!$started) {
          $c = "$bool_op (LOWER($table.hashtags) REGEXP '([[:blank:][:punct:]]|^)" . $link->real_escape_string(trim(strtolower($k))) . "([[:blank:][:punct:]]|$)' ";
          $bool_op = $_GET['bool_op'];
        }
        else
          $c = $c . " OR LOWER($table.hashtags) REGEXP '([[:blank:][:punct:]]|^)" . $link->real_escape_string(trim(strtolower($k))) . "([[:blank:][:punct:]]|$)' ";
        $started = true;
      }
      $condition = $condition . " $c) ";
    }
    if ($_GET['any_mentions']) {
      ...
      foreach ($tmp as $k) {
        $k = '@' . trim($k, '@');
        if (!$started) {
          $c = "$bool_op (LOWER(user_mentions) REGEXP '" . $link->real_escape_string(trim($k)) . "([[:blank:]]|$)' ";
          $bool_op = $_GET['bool_op'];
        }
        else
          $c = $c . " OR LOWER(user_mentions) REGEXP '" . $link->real_escape_string(trim($k)) . "([[:blank:]]|$)' ";
        $started = true;
      }
      $condition = $condition . " $c) ";
    }
    if ($_GET['all_mentions']) {
      ...
      foreach ($tmp as $k) {
        ...
        if (!$started) {
          ...
          $c = $c . " AND LOWER(user_mentions) REGEXP '" . $link->real_escape_string(trim($k)) . "([[:blank:]]|$)' ";
        $started = true;
      }
      $condition = $condition . " $c) ";
    }
    ...
    if ($_GET['any_keywords']) {
      ...
      foreach ($tmp as $k) {
        if (!$started) {
          $c = "$bool_op (LOWER(clear_text) like '%" . $link->real_escape_string(trim($k)) . "%' ";
          $bool_op = $_GET['bool_op'];
        }
        else
          $c = $c . " OR LOWER(clear_text) like '%" . $link->real_escape_string(trim($k)) . "%' ";
        $started = true;
      }
      $condition = $condition . " $c) ";
    }
    if ($_GET['any_keywords_2']) {
      ...
      foreach ($tmp as $k) {
        if (!$started) {
          $c = "$bool_op (LOWER(clear_text) REGEXP '([[:blank:][:punct:]]|^)" . $link->real_escape_string(trim($k)) . "([[:blank:][:punct:]]|$)' ";
          $bool_op = $_GET['bool_op'];
        }
        else
          $c = $c . " OR LOWER(clear_text) REGEXP '([[:blank:][:punct:]]|^)" . $link->real_escape_string(trim($k)) . "([[:blank:][:punct:]]|$)' ";
        $started = true;
      }
      $condition = $condition . " $c) ";
    }
    if ($_GET['all_keywords']) {
      ...
      foreach ($tmp as $k) {
        if (!$started) {
          ...
          $c = $c . " AND LOWER(clear_text) like '%" . $link->real_escape_string(trim($k)) . "%' ";
        $started = true;
      }
      $condition = $condition . " $c) ";
    }
    if ($_GET['all_keywords_2']) {
      ...
      foreach ($tmp as $k) {
        if (!$started) {
          $c = "$bool_op (LOWER($table.clear_text) REGEXP '([[:blank:][:punct:]]|^)" . $link->real_escape_string(trim($k)) . "([[:blank:][:punct:]]|$)' ";
          $bool_op = $_GET['bool_op'];
        }
        else
          $c = $c . " AND LOWER($table.clear_text) REGEXP '([[:blank:][:punct:]]|^)" . $link->real_escape_string(trim($k)) . "([[:blank:][:punct:]]|$)' ";
        $started = true;
      }
      $condition = $condition . " $c) ";
    }
    ...
    if ($_GET['from_accounts']) {
      ...
      foreach ($tmp as $k) {
        if (!$started) {
          $c = "$bool_op ((LOWER($table.user_screen_name)='" . $link->real_escape_string(trim($k)) . "') ";
          $bool_op = $_GET['bool_op'];
        }
        else
          $c = $c . " OR (LOWER($table.user_screen_name)='" . $link->real_escape_string(trim($k)) . "') ";
        $started = true;
      }
      $condition = $condition . " $c) ";
    }
    if ($_GET['in_reply_to_tweet_id']) {
      ...
      $c = "$bool_op ($table.in_reply_to_tweet='{$_GET['in_reply_to_tweet_id']}'";
      $bool_op = $_GET['bool_op'];
      $condition = $condition . " $c) ";
    }
    ...
    if ($_GET['location']) {
      ...
      foreach ($tmp as $k) {
        if (!$started) {
          $c = "$bool_op ((LOWER(location_fullname) like '%" . $k . "%' OR LOWER(location_name) like '%" . $k . "%' OR LOWER(user_location) like '%" . $k . "%' OR LOWER(user_timezone) like '%" . $k . "%')";
          $bool_op = $_GET['bool_op'];
        }
        else
          $c = $c . " OR (LOWER(location_fullname) like '%" . $k . "%' OR LOWER(location_name) like '%" . $k . "%' OR LOWER(user_location) like '%" . $k . "%' OR LOWER(user_timezone) like '%" . $k . "%')";
        $started = true;
      }
      $condition = $condition . " $c) ";
    }
  ...
  $params = "";
  if ($retweets)
    $name = $name . " of tweets+retweets ";
  elseif ($unique_tweeters)
    $name = $name . " of unique tweeters ";
  elseif ($normalized_tweets)
    $name = $name . " of original tweets per tweeter";
  elseif ($normalized_retweets)
    $name = $name . " of (tweets+retweets) per tweeter";
  elseif ($relative_tweeters)
    $name = $name . " relative impact (tweeters)";
  elseif ($relative_tweets)
    $name = $name . " relative impact (original tweets)";
  elseif ($relative_retweets)
    $name = $name . " relative impact (tweets+retweets)";
  else
    $name = $name . " of tweets ";
  foreach (array_keys($_GET) as $g) {
    if ($_GET[$g] && in_array($g, $g_params))
      $params = $params . " $g:" . $_GET[$g] . ",";
  }
  $params = rtrim($params, ",");
  if ($params)
    $name = "[$params]";
  ...
  if ($drill_level == "years") {
    $param1 = "CONVERT_TZ(DATE_FORMAT(date_time,'%Y-01-01 00:00:00')";
    $param2 = "DATE_FORMAT(date_time,'%Y-01-01 00:00:00')";
  }
  elseif ($drill_level == "months") {
    $param1 = "CONVERT_TZ(DATE_FORMAT(date_time,'%Y-%m-01 00:00:00')";
    $param2 = "DATE_FORMAT(date_time,'%Y-%m-01 00:00:00')";
  }
  elseif ($drill_level == "days") {
    $param1 = "CONVERT_TZ(DATE_FORMAT(date_time,'%Y-%m-%d 00:00:00')";
    $param2 = "tweet_date";
  }
  elseif ($drill_level == "hours") {
    $param1 = "CONVERT_TZ(DATE_FORMAT(date_time,'%Y-%m-%d %H:00:00')";
    $param2 = "DATE_FORMAT(date_time,'%Y-%m-%d %H:00:00')";
  }
  elseif ($drill_level == "minutes") {
    $param1 = "CONVERT_TZ(DATE_FORMAT(date_time,'%Y-%m-%d %H:%i:00')";
    $param2 = "DATE_FORMAT(date_time,'%Y-%m-%d %H:%i:00')";
  }
  elseif ($drill_level == "seconds") {
    $param1 = "CONVERT_TZ(DATE_FORMAT(date_time,'%Y-%m-%d %H:%i:%S')";
    $param2 = "date_time";
  }
  else {
    $param1 = "tweet_date";
  ...
  if (!$include_retweets)
    $tweet_count = "count(tweet_id)+sum(retweets)";
  else
    $tweet_count = "count(tweet_id)";
  ...
  elseif ($normalized_retweets)
    $query = $query . $link2 . "SELECT UNIX_TIMESTAMP($param1, '+00:00', @@session.time_zone))*1000,ROUND(($tweet_count)/(count(distinct user_id))) from $table $condition group by $param2";
  elseif ($normalized_tweets)
  ...
  elseif ($relative_retweets)
    $query = $query . $link2 . "SELECT UNIX_TIMESTAMP($param1, '+00:00', @@session.time_zone))*1000,100*(($tweet_count)/Nr_Twitter_Users(YEAR(date_time))) from $table $condition group by $param2";
  elseif ($relative_tweets)
  ...
  elseif ($retweets)
    $query = $query . $link2 . "SELECT UNIX_TIMESTAMP($param1, '+00:00', @@session.time_zone))*1000,$tweet_count from $table $condition group by $param2";
  else
  ...
  if ($result = $link->query($query)) {
    if (!$result->num_rows)
      die("No results in the database matched your query.<br>\n");
  ...
  if ($result = $link->query($qry)) {
    ...
    if ($flags_arr) {
      $flags_data = json_decode($flags_arr[0], true);
    }
  }
  $link->close();
  ...
  if (isset($flags_data['flags'])) {
    foreach ($flags_data['flags'] as $pd) {
      if (($to && $pd['date_and_time'] > $to) || ($from && $pd['date_and_time'] < $from))
        continue;
  ...
  if ($_GET['point'] > 1) {
    ...
    if ($drill_level == "years")
      $point = gmdate("Y-01-01 00:00:00", $point);
    elseif ($drill_level == "months")
      $point = gmdate("Y-m-01 00:00:00", $point);
    elseif ($drill_level == "days")
      $point = gmdate("Y-m-d 00:00:00", $point);
    elseif ($drill_level == "hours")
      $point = gmdate("Y-m-d H:00:00", $point);
    elseif ($drill_level == "minutes")
      $point = gmdate("Y-m-d H:i:00", $point);
    elseif ($drill_level == "seconds")
      $point = gmdate("Y-m-d H:i:s", $point);
  }
  else
    $point = 1;
  $started = false;
  ...
  if ($enddate)
    $to = trim("$enddate $endtime");
  if ($from)
    $condition = $condition . " AND $table.date_time>='$from'";
  if ($to)
    $condition = $condition . " AND $table.date_time<'$to'";
  ...
  if ($_GET['image'])
    $condition = $condition . " AND ($table.expanded_links like '%" . $_GET['image'] . "%' OR $table.media_link like '%" . $_GET['image'] . "%')";
  if ($_GET['video'] || $_GET['image2'])
    $condition = $condition . " AND ($table.expanded_links like '%" . $_GET['video'] . "%' OR $table.media_link like '%" . $_GET['video'] . "%' OR $table.media_link like '%" . $_GET['image2'] . "%')";
  if ($_GET['link'])
    $condition = $condition . " AND ($table.expanded_links='" . $_GET['link'] . "')";
  ...
  if ($_GET['types'] == "some") {
    if ($_GET['bool_op'] == "NOT" || $_GET['bool_op'] == "  OR ") {
      ...
      $bool_op = " AND (";
    if ($_GET['image_tweets']) {
      $condition = $condition . " $bool_op $table.has_image=1 ";
      $bool_op = $_GET['bool_op'];
    }
    if ($_GET['video_tweets']) {
      $condition = $condition . " $bool_op $table.has_video=1 ";
      $bool_op = $_GET['bool_op'];
    }
    if ($_GET['link_tweets']) {
      $condition = $condition . " $bool_op $table.has_link=1 ";
      $bool_op = $_GET['bool_op'];
    }
    if ($_GET['original_tweets']) {
      $condition = $condition . " $bool_op ($table.is_retweet<>1) ";
      $bool_op = $_GET['bool_op'];
    }
    if ($_GET['retweet_tweets']) {
      $condition = $condition . " $bool_op ($table.is_retweet=1) ";
    ...
    if ($_GET['response_tweets']) {
      $condition = $condition . " $bool_op (($table.in_reply_to_tweet is not null OR $table.in_reply_to_user is not null) AND (is_reply=1 OR in_reply_to_user is not null)) ";
      $bool_op = $_GET['bool_op'];
    }
    if ($_GET['quoting_tweets']) {
      $condition = $condition . " $bool_op (is_quote=1) ";
      $name = $name . " (quoting a tweet)";
      $bool_op = $_GET['bool_op'];
    }
    if ($_GET['referenced_tweets']) {
      $condition = $condition . " $bool_op (is_referenced=1) ";
      $name = $name . " (referenced by another tweet)";
      $bool_op = $_GET['bool_op'];
    }

    if ($_GET['mentions_tweets']) {
      $condition = $condition . " $bool_op ($table.clear_text like '@%') ";
      $bool_op = $_GET['bool_op'];
    }
    if ($_GET['responded_tweets']) {
      $condition = $condition . " $bool_op (replies is not null AND replies>0) ";
    ...
    if ($_GET['user_verified']) {
      $condition = $condition . " $bool_op $table.user_verified=1 ";
      $bool_op = $_GET['bool_op'];
    }
    if ($_GET['min_retweets']) {
      $condition = $condition . " $bool_op $table.retweets>=" . $_GET['min_retweets'] . " ";
      $bool_op = $_GET['bool_op'];
    }
    if ($_GET['any_hashtags']) {
      $_GET['any_hashtags'] = str_replace("#", " ", $_GET['any_hashtags']);
    ...
    if ($_GET['any_keywords']) {
      ...
      foreach ($tmp as $k) {
        if (!$started) {
          $c = "$bool_op (LOWER($table.clear_text) like '%" . $link->real_escape_string(trim($k)) . "%' ";
          $bool_op = $_GET['bool_op'];
        }
        else
          $c = $c . " OR LOWER($table.clear_text) like '%" . $link->real_escape_string(trim($k)) . "%' ";
        $started = true;
      }
      $condition = $condition . " $c) ";
    }
    ...
    if ($_GET['all_keywords']) {
      ...
      foreach ($tmp as $k) {
        if (!$started) {
          ...
          $c = $c . " AND LOWER($table.clear_text) like '%" . $link->real_escape_string(trim($k)) . "%' ";
        $started = true;
      }
      $condition = $condition . " $c) ";
    }
    ...
    if ($_GET['location']) {
      ...
      foreach ($tmp as $k) {
        if (!$started) {
          $c = "$bool_op ((LOWER($table.location_fullname) like '%" . $k . "%' OR LOWER($table.location_name) like '%" . $k . "%' OR LOWER(" . $table . ".user_location) like '%" . $k . "%')";
          $bool_op = $_GET['bool_op'];
        }
        else
          $c = $c . " OR (LOWER($table.location_fullname) like '%" . $k . "%' OR LOWER($table.location_name) like '%" . $k . "%' OR LOWER($table.user_location) like '%" . $k . "%' OR LOWER($table.user_timezone) like '%" . $k . "%')";
        $started = true;
      }
      $condition = $condition . " $c) ";
    }
  ...
  if ($tops) {
    ...
    if (!$include_retweets)
      $order = "sum($table.retweets)";
    else
      $order = "count(tweet_id)";
    ...
    if ($result = $link->query($query)) {
      ...
      if (!$total_rows)
        die("No results in the database matched your query. - <a href='#' onclick=javascript:GetDetails('$url')><small>Back to the main list</small></a></center><br>\n");
    }
    else
      die("Error in query: " . $link->error . ": $query");
    if ($top_images)
      $data = "<center><b>Top images from $table</b> ";
    elseif ($top_videos)
      $data = "<b>Top videos from $table</b> ";
    elseif ($top_links)
      $data = "<b>Top links from $table</b> ";
    if ($shared)
      $data = $data . " <i>sorted by the number of shares</i> ";
    else
      $data = $data . " <i>sorted by the number of retweets</i> ";
    ...
    if ($top_images)
      $data = $data . "<td><b>Image</b> (click to see list of tweets)</td>";
    elseif ($top_videos)
      $data = $data . "<td><b>Video</b></td>";
    else
      $data = $data . "<td><b>Link</b></td>";
    if ($shared)
      $data = $data . "<td><b>Shares</b></td></tr>";
    else
      $data = $data . "<td><b>Retweets</b></td></tr>";
    $cnt = 0;
    ...
    while ($row = $result->fetch_array()) {
      ...
      if ($top_images) {
        ...
        $temp_list = explode(" ", trim($row[1]));
        $links = "";
        foreach ($temp_list as $temp_link) {
          if (strlen($temp_link) > 15)
            $links = $links . "<a href='" . $row[0] . "' target=_blank><img src='$temp_link' height=250></a><br> <br>";
        }
      ...
      elseif ($top_videos) {
        if ($row[1]) {
          ...
          $img = 0;
          $temp_list = explode(" ", trim($row[0]));
        }
        $links = "";
        foreach ($temp_list as $temp_link) {
          $links = $links . "<a href='" . $row[0] . "' target=_blank>" . image_exists($img, $temp_link) . $row[0] . "</a>";
        }
        ...
        $links = "";
        foreach ($temp_list as $temp_link) {
          $links = $links . "<a href='$temp_link' target=_blank>$temp_link</a> ";
        }
    ...
    $order = "";
    if ($_GET['order_d'])
      $order = "order by $table.date_time";
    elseif ($_GET['order_u'])
      $order = "order by $table.user_screen_name";
    elseif ($_GET['order_t'])
      $order = "order by $table.clear_text";
    elseif ($_GET['order_r'])
      $order = "order by $table.retweets";
    elseif ($_GET['order_q'])
      $order = "order by $table.quotes";
    elseif ($_GET['order_f'])
      $order = "order by $table.favorites";
    elseif ($_GET['order_ri'])
      $order = "order by ($table.retweets/Nr_Twitter_Users(YEAR($table.date_time)))";
    elseif ($_GET['order_rs'])
      $order = "order by $table.replies";
    elseif ($_GET['order_s'])
      $order = "order by $table.source";
    elseif ($_GET['order_l'])
      $order = "order by $table.tweet_language";
    elseif ($_GET['order_i'])
      $order = "order by $table.media_link";
    elseif ($_GET['order_v'])
      $order = "order by $table.user_verified";
    ...
    if ($_GET['asc'])
      ...
      $order = "$order DESC";
    if ($order)
      $order = $order . ", $table.date_time ASC";
    else
      $order = "order by $table.date_time ASC";
    if ($_GET['limit'] == -1)
      $limit = 1000000;
    elseif ($_GET['limit'])
      $limit = $_GET['limit'];
    ...
    if (!$_GET['export']) {
      ...
      $heading = $heading . "<table style='font-size:8pt; background-color:#FFFFFF; width: 1200px; table-layout: fixed;'><tr>" .
        "<td width=30>#</td>" .
        "<td width=110 style='white-space: nowrap;'><a href='#' onclick=javascript:GetDetails('$url&asc=$oppasc&order_d=1')>Date & Time" . arrowdir($oppasc, 'd') . "</a></td>" .
        "<td width=140 style='white-space: nowrap;'><a href='#' onclick=javascript:GetDetails('$url&asc=" . $oppasc . "&order_u=1')>Tweeter's details " . arrowdir($oppasc, 'u') . "</a></td>" .
        "<td width=150><a href='#' onclick=javascript:GetDetails('$url&asc=" . $oppasc . "&order_t=1')>Tweet text" . arrowdir($oppasc, 't') . "</a></td>" .
        "<td width=80 style='white-space: nowrap;'><a href='#' onclick=javascript:GetDetails('$url&asc=" . proper_order("min_retweets") . "&order_r=1')>Retweets" . arrowdir($oppasc, 'r') . "</a></td>" .
        "<td width=70 style='white-space: nowrap;'><a href='#' onclick=javascript:GetDetails('$url&asc=$oppasc&order_q=1')>Quotes" . arrowdir($oppasc, 'q') . "</a></td>" .
        "<td width=80 style='white-space: nowrap;'><a href='#' onclick=javascript:GetDetails('$url&asc=$oppasc&order_f=1')>Favorites" . arrowdir($oppasc, 'f') . "</a></td>$ri" .
        "<td width=70 style='white-space: nowrap;'><a href='#' onclick=javascript:GetDetails('$url&asc=" . proper_order("responded_tweets") . "&order_rs=1')>Replies" . arrowdir($oppasc, 'rs') . "</a></td>" .
        "<td width=80 style='white-space: nowrap;'><a href='#' onclick=javascript:GetDetails('$url&asc=" . $oppasc . "&order_s=1')>Source" . arrowdir($oppasc, 's') . "</a></td>" .
        "<td width=50 style='white-space: nowrap;'><a href='#' onclick=javascript:GetDetails('$url&asc=" . $oppasc . "&order_l=1')>Lang" . arrowdir($oppasc, 'l') . "</a></td>" .
        "<td width=120 style='white-space: nowrap;'><a href='#' onclick=javascript:GetDetails('$url&asc=" . proper_order("user_verified") . "&order_v=1')>Verified user" . arrowdir($oppasc, 'v') . "</a></td>" .
        "<td width=200><a href='#' onclick=javascript:GetDetails('$url&asc=" . proper_order("image_retweets") . "&order_i=1')>Image (if any)" . arrowdir($oppasc, 'i') . "</a></td></tr>\n";
      $url = $orig_url;
    }
    else {
      ...
      if (!$_GET['export']) {
        ...
        if ($cnt + 1 >= $p && $cnt < $pp) {
          ...
          if ($show_relative_impact)
            $ri = "<td><center>{$row['relative_impact']}</center></td>";
    ...
    if ($retweets) {
      if (!$include_retweets)
        $addretweets = "sum(retweets)";
      else
        $addretweets = "count(tweet_id)";
      $cnd = $condition;
      if ($point > 1) {
        if ($drill_level == "years")
      ...
      if ($result = $link->query($qry)) {
        $row = $result->fetch_array();
        $total_retweets = $row[0];
      }
      if ($debug && $_SESSION[basename(__DIR__) . 'email'] == $admin_email) {
        echo "qry:$qry - retweets: $total_retweets <br>\n";
      }
    }
    ...
    if (!$_GET['export']) {
      $data = $data . "</table><hr>";
      if ($total_rows < $pp) {
        $results = "<ul><li>Case id: <b>$table</b></li><li> Title: <b>" . $cases[$table]['name'] . "</b></li>";
        $show_query = ($cases[$table]['query'] != 'N/A' && $cases[$table]['query'] != '');
        $show_period = (($cases[$table]['from'] && $cases[$table]['from'] != '0000-00-00' && $cases[$table]['from'] != '0000-00-00 00:00:00') || ($cases[$table]['to'] && $cases[$table]['to'] != '0000-00-00' && $cases[$table]['to'] != '0000-00-00 00:00:00'));
        if ($show_query) {
          $results .= "<li>Search query: <b>" . $cases[$table]['query'] . "</b></li>";
        }
        if ($show_period) {
          $results .= "<li>Period: from (<b>" . $cases[$table]['from'] . "</b>) to (<b>" . $cases[$table]['to'] . "</b>)</li>";
        }
        if ($cases[$table]['details'])
          $results .= "<li>Details:<b> " . $cases[$table]['details'] . "</b></li>";
        ...
        if (!$include_retweets)
          $results = $results . " [" . ($total_retweets + $total_rows) . " with retweets] ";

        if ($p > $per_page) {
          if (!(($p - $per_page == 1) && ($pp - $per_page == $per_page)))
            $ppstr = "&p=" . ($p - $per_page) . "&pp=" . ($pp - $per_page);
          else
            $ppstr = "";
        ...
        if (!(($p - $per_page == 1) && ($pp - $per_page == $per_page)))
          ...
          $ppstr = "";
        if ($p > $per_page)
          $results = $results . "<center> <a href='#' onclick=javascript:GetDetails('$url$ppstr')> &lt; </a>  &nbsp;&nbsp; <a href='#' onclick=javascript:GetDetails('$url')><small>Back to first page</small></a> ";
  ...
  if (isset($link->server_info))
    $link->close();
  $slide_file = file_get_contents("templates/slideshow.html");
  $slide_file = str_replace("<!--case-->", $cases[$table]['name'], $slide_file);
  $query_text = ($cases[$table]['query'] != 'N/A' && $cases[$table]['query'] != '') ? " Search query: <b>" . $cases[$table]['query'] . "</b>" : "";
  $slide_file = str_replace("<!--title-->", $query_text, $slide_file);
...
function put_img($link, $img)
{
  if ($img) {
    ...
    foreach ($img_list as $im) {
      $images = $images . "<img src='$im' width=200><br> <br>";
    }
...
function profile_location($l1, $l2)
{
  $l = "";
  if ($l1)
    $l = "<br><b>Based in:</b> $l1";
  if ($l2)
    $l = "$l<br><b>Timezone:</b> $l2";
  return $l;
}
...
function answer($verified)
{
  if ($verified)
    return "Yes";
  return "No";
}
function answer2($location)
{
  if ($location == "NULL")
    return "";
  return $location;
