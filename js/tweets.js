...
function toggle_login(preserve) {
	...
	$.ajax({
		url: "login.php?action=toggle_login&page=tweets&login=1",
...
function showtip(field) {
	$.ajax({
		url: 'login.php?action=tip&field=' + field,
...
function check_id() {
	...
	if (!id.value.match(/^[0-9a-zA-Z_]+$/)) {
		id_comment.innerHTML = "<br><font color='red'><small>The ID can only contain alphanumeric characters or underscores (no spaces or other special characters)</small></font>";
...
function case_proc(action, case_id, email) {
	...
	if (action == "more_info") {
		...
		if (/[^a-zA-Z0-9_]/.test(case_id)) {
			alert('The case ID has to be alphanumeric or underscore (only alphabets, numbers, or underscores, and no spaces)');
			return false;
		}
		...
		var ajax = $.ajax({
			type: 'POST', url: 'login.php',
			data: {
				action: action, email: email, case_id: case_id, case_name: case_name, case_platform: case_platform,
		...
		$.ajax({
			url: 'login.php?action=delete_case&case_id=' + case_id + '&email=' + email,
			complete: function (response) {
				$('#chartcontainer').html(response.responseText);
				toggle_login();
			},
		...
		$.ajax({
			url: 'login.php?action=empty_case&case_id=' + case_id + '&email=' + email,
			complete: function (response) {
				$('#chartcontainer').html(response.responseText);
				toggle_login();
			},
		...
		$.ajax({
			url: 'login.php?action=edit_case&case_id=' + case_id + '&email=' + email,
			complete: function (response) {
				$('#chartcontainer').html(response.responseText);
			},
			error: function () {
		...
		$.ajax({
			url: 'login.php?action=delete_account&email=' + email,
			complete: function (response) {
				$('#chartcontainer').html(response.responseText);
				toggle_login();
			},
...
function show_or_hide(elements, visible) {
	...
	for (var index = 0; index < elements2.length; index++) {
		...
		if (elements.id == "specify_types" && visible == "none") {
			...
			document.getElementById("all_keywords_2").value = "";
...
$(document).ready(function () {
	...
	for (var key in searchParams) {
		if (searchParams.hasOwnProperty(key)) {
			...
			if (key === 'table' || key === 'id' || key === 'point' || key === 'asc' || key === 'order_d' || key === 'demo' || key === 'refresh') continue;
			hasParams = true;
			var el = document.getElementById(key);
			if (el) {
				if (el.type === 'checkbox' || el.type === 'radio') {
					el.checked = (value == '1' || value == 'true' || el.value == value);
				} else {
					el.value = value;
				}
			} else {
	...
	if (searchParams['types'] === 'some') {
		var specificTypes = document.getElementById('specific_types');
		if (specificTypes) specificTypes.checked = true;
		$("#specify_types").show();
		$(".slidingDiv2").show();
	}

	if (searchParams['languages'] === 'some') {
		var l = document.getElementById('some_langs');
		if (l) l.checked = true;
	}

	if (searchParams['sources'] === 'some') {
		var s = document.getElementById('some_sources');
		if (s) s.checked = true;
	}
});
...
function visualize(refresh = "") {
	...
	try {
		...
		qry['all_keywords_2'] = document.getElementById("all_keywords_2").value.toLowerCase();
		...
		if (qry['types'] == "all" || (qry['image_tweets'] && qry['video_tweets'] && qry['link_tweets'] && qry['original_tweets'] && qry['retweet_tweets']
			...
			&& qry['exact_phrase'] == '' && qry['from_accounts'] == '' && qry['in_reply_to_tweet_id'] == '' && qry['location'] == '' && qry['min_retweets'] == '' && qry['user_verified'])) { qry['types'] = ""; }
		else qry['types'] = "some";
	...
	var searchParams = getQueryParams();
	if (searchParams['id']) browserUrl += 'id=' + encodeURIComponent(searchParams['id']) + '&';
	if (searchParams['table']) browserUrl += 'table=' + encodeURIComponent(searchParams['table']) + '&';

	for (var key in qry) {
		if (qry.hasOwnProperty(key)) {
			if (qry[key]) {
				url = url + '&' + key + '=' + encodeURIComponent(qry[key]);
				if (key !== 'table') {
					browserUrl += key + '=' + encodeURIComponent(qry[key]) + '&';
				}
			}
		}
	}
	...
	$.ajax({
		...
		complete: function (response) {
			...
			if (($('input[name="toptweets"]:checked').val())) {
				...
				$("#loadingtweets").show();
				$.ajax({
					url: url + '&point=1&asc=1&order_d=1&refresh=' + refresh,
					complete: function (response) {
						$('#tweetcontainer').html(response.responseText);
						jQuery("#loadingtweets").hide();
						$("#loadingchart").hide();
