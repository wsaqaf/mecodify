...
$(document).ready(function () {
	...
	for (var key in searchParams) {
		if (searchParams.hasOwnProperty(key)) {
			...
			if (key === 'table' || key === 'id' || key === 'overview') continue;
...
function visualize(network) {
	...
	var fetchParams = url.split('?')[1];
	if (fetchParams) {
		var fetchPairs = fetchParams.split('&');
		for (var i = 0; i < fetchPairs.length; i++) {
			var kv = fetchPairs[i].split('=');
			if (kv.length !== 2) continue;
			browserUrl += kv[0] + '=' + kv[1] + '&';
		}
	}
...
function showkumu() {
	...
	$.ajax({
		type: 'POST', url: url,
		complete: function (response) {
			if (response.status == 200) { $('#kumu_users').html("<a href='" + url + "'>Tweeter CSV file</a><br>"); }
		},
	...
	$.ajax({
		type: 'POST', url: url0,
		complete: function (response) {
			if (response.status == 200) { $('#kumu_retweets').html("<a href='" + url0 + "'>Retweets CSV file</a><br>"); }
		},
	...
	$.ajax({
		type: 'POST', url: url0n,
		complete: function (response) {
			if (response.status == 200) { $('.network_retweets').show(); }
		},
	...
	$.ajax({
		type: 'POST', url: url1,
		complete: function (response) {
			if (response.status == 200) { $('#kumu_responses').html("<a href='" + url1 + "'>Response CSV file</a><br>"); }
		},
	...
	$.ajax({
		type: 'POST', url: url2,
		complete: function (response) {
			if (response.status == 200) { $('#kumu_mentions').html("<a href='" + url2 + "'>Mentions CSV file</a><br>"); }
		},
	...
	$.ajax({
		type: 'POST', url: url3,
		complete: function (response) {
			if (response.status == 200) { $('#kumu_top_10000').html("<a href='" + url3 + "'>Top 10000 tweets CSV file</a><br>"); }
		},
	...
	$.ajax({
		type: 'POST', url: url4,
		complete: function (response) {
			if (response.status == 200) { $('#kumu_top_5000').html("<a href='" + url4 + "'>Top 5000 CSV file for Kumu import</a><br>"); }
		},
	...
	$.ajax({
		type: 'POST', url: url5,
		complete: function (response) {
			if (response.status == 200) { $('#kumu_top_1000').html("<a href='" + url5 + "'>Top 1000 CSV file for Kumu import</a><br>"); }
		},
