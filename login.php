...
if ($upload_csv_files) {
  ...
  $blank_values = "N/A";
  $csv_message = "<br><span style='color:blue'><b>Note:</b> You can use the <a href='https://github.com/wsaqaf/xscraper' target='_blank'>xscraper Chrome Extension</a> to generate CSV files for this case.</span><br>";
}
else {
  $hidden_style = "";
  $blank_values = "";
...
function submit_case($replace)
{
  ...
  if ($replace) {
    ...
    if ($_POST['case_from'] == "0000-00-00 00:00:00")
      $_POST['case_from'] = "";
    if ($_POST['case_to'] == "0000-00-00 00:00:00")
      $_POST['case_to'] = "";
...
function get_from_db($email = "", $menu = false, $case = "")
{
  global $link;
  global $login_str;
  global $admin_email;
  global $allow_new_cases;
  if (!$case) {
    echo "Please select a case first";
    return;
  }
  $cond = "";
  ...
  if ($email) {
    ...
    if ($case)
      $condition .= " AND id='$case' $cond";
  }
  elseif ($case)
    $condition = "where id='$case' $cond";
  else
    $condition = "";
  ...
  if ($menu) {
    ...
    if ($allow_new_cases)
      ...
      $list .= "<div style='text-align:center'>Select one of the cases to visualise.</div><br>";
  }
  return $list;
}
...
function edit_from_db($email, $menu, $case)
{
  ...
  if ($email) {
    $condition = "where creator='$email'";
    if ($case)
      $condition .= " AND id='$case' ";
  }
  elseif ($case)
    $condition = "where id='$case' ";
  else
    $condition = "";
  $query = "SELECT * from cases $condition";
  $output = "";
...
{
  global $link;
  if (!preg_match('/^[a-zA-Z0-9\@\.\-\_]+$/', $email) or !preg_match('/^[a-zA-Z0-9\@\!\.\-\_]+$/', $password))
    return false;
  $hash = get_pw($email);
...
function del_account($creator)
{
  ...
  if ($demo_config['enabled'] && $_SESSION[basename(__DIR__) . 'email'] == $demo_config['email']) {
    echo "Deleting DEMO profile is not permitted";
    return;
  }
...
function get_pw($email)
{
  ...
  if ($result = $link->query($query)) {
    if (!$result->num_rows)
      die("There is no account with email ($email). You can create a new account or email <a href='mailto:$admin_email'>$admin_email</a> for support.");
  }
  else
    die("Error in query: " . $link->error . ": $query");
  $row = $result->fetch_assoc();
...
function verify_code($email, $code = "")
{
  ...
  if ($result = $link->query($query)) {
    if (!$result->num_rows)
      die("The code could not be verified. Please try again or email <a href='mailto:$admin_email'>$admin_email</a> for support.");
  }
  else
    die("Error in query: " . $link->error . ": $query");
...
function create_account($replace)
{
  ...
  if (!($result = $link->query($query)))
    die("Could not update the database. Please contact admin! Error in query: " . $link->error . ": $query");
  email_admin("", lst($_POST));
  return $returned;
}
