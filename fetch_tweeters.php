...
elseif ($responses) {
  if ($table) {
    if (file_exists("tmp/network/$table" . "_" . "$level.csv")) {
      ...
      echo "The results don't have enough connections to form network graphs. Try fetching more records...";
  }
}
elseif ($mentions) {
  if ($table) {
...
elseif ($user_screen_name)
  show_profile($rank, $table, $user_screen_name);
...
function show_profile($rank, $table, $user_screen_name)
{
  ...
  if (!$row || !$row['a']) {
    ...
    if ($result = $link->query($query)) {
      if (!$result->num_rows)
        echo "<tr><td>The account [$user_screen_name] may have been suspended or deleted.</tr></td>";
      $total = $result->num_rows;
    }
    else
      die("Error in query: " . $link->error . ": $query");
  ...
  if ($row['a'] === 0 || $row['a']) {
    ...
    if ($row['user_verified'])
      echo "<img src='images/twitter-verified.jpg' width=100 alt='$user_screen_name verified' style='display: inline-block;'>\n";
    echo "<a href='https://www.twitter.com/$user_screen_name' target=_blank><b>{$row['user_name']} ($user_screen_name)</b></a></td></tr>\n";
    if ($row['user_bio'])
      echo "<tr><td>Bio</td><td><b>{$row['user_bio']}</b></td</tr>\n";
    if ($row['user_location'])
      echo "<tr><td>Based in <b></td><td></b>{$row['user_location']}</b> <i>(as indicated in profile)</i></td></tr>";
    if ($row['user_timezone'])
      echo "<tr><td>Timezone: </td><td><b>{$row['user_timezone']}</b></td></tr>";
...
function get_top($type, $limit)
{
  ...
  if ($_GET['location']) {
    ...
    $c = "";
    $started = false;
    if ($type == "followers" || $type == "all_mentions")
      $t = "users_" . $table;
    elseif ($type == "quotes")
      $t = "k1";
    else
    ...
    foreach ($tmp as $k) {
      $k = strtolower($k);
      if (!$started)
        $c = "AND ((LOWER($t.user_location) like '%" . $k . "%' OR LOWER($t.user_timezone) like '%" . $k . "%')";
      else
        $c = " OR (LOWER($t.user_location) like '%" . $k . "%' OR LOWER($t.user_timezone) like '%" . $k . "%')";
    }
  ...
  if ($_GET['bio']) {
    ...
    foreach ($tmp as $k) {
      $k = strtolower($k);
      if (!$started)
        $c = "AND (LOWER($t.user_bio) like '%" . $k . "%' ";
      else
        $c = " OR LOWER($t.user_bio) like '%" . $k . "%' ";
    }
    $condition = $condition . " $c) ";
  }
  /* to be implemented
  ...
  if ($type == "followers") {
    ...
    // Fallback if users_table is empty, missing columns, or all users are out of search results
    connect_mysql();

    $has_user_table = $link->query("SHOW TABLES LIKE 'users_$table'")->num_rows > 0;
    $has_fol_in_user = false;
    if ($has_user_table) {
      $has_fol_in_user = $link->query("SHOW COLUMNS FROM users_$table LIKE 'user_followers'")->num_rows > 0;
    }

    $check_users = null;
    if ($has_user_table && $has_fol_in_user) {
      $check_users = $link->query("SELECT user_screen_name FROM users_$table WHERE user_followers IS NOT NULL AND user_followers > 0 LIMIT 1");
    }

    if (!$has_user_table || !$has_fol_in_user || !$check_users || $check_users->num_rows == 0) {
      // No valid followers data available
      die("Follower data is not available for this case. Try another filter.");
    }
    $query = "$qry $condition ORDER BY followers DESC";
  }
  elseif ($type == "retweets") {
    ...
    $condition = preg_replace("/\s*WHERE/i", " AND ", $condition);
    $condition = "WHERE (is_retweet<>1 OR is_retweet IS NULL) " . $condition . " ";
  ...
  elseif ($type == "responses") {
    ...
    if ($_GET['export'])
      $qry = "SELECT user_screen_name,user_name AS full_name,count(tweet_id) AS replies,user_verified, user_location, user_bio,CONCAT('https://twitter.com/',user_screen_name) AS user_twitter_page  FROM $table ";
    else
      $qry = "SELECT user_screen_name,count(tweet_id) AS replies FROM $table ";
    $condition = preg_replace("/\s*WHERE/i", " AND ", $condition);
    $condition = "WHERE is_reply=1 " . $condition;
  ...
  elseif ($type == "responders") {
    ...
    // Priority: responses_to_tweeter > replies > counting in_reply_to_user_screen_name
    $has_resp_col = $link->query("SHOW COLUMNS FROM `$table` LIKE 'responses_to_tweeter'")->num_rows > 0;
    $has_repl_col = $link->query("SHOW COLUMNS FROM `$table` LIKE 'replies'")->num_rows > 0;

    $use_resp = false;
    if ($has_resp_col) {
      $check = $link->query("SELECT user_screen_name FROM `$table` WHERE responses_to_tweeter > 0 LIMIT 1");
      if ($check && $check->num_rows > 0)
        $use_resp = true;
    }
    if ($use_resp) {
      if ($_GET['export'])
        $qry = "SELECT user_screen_name,user_name AS full_name,responses_to_tweeter AS repliers,mentions_of_tweeter,user_verified,user_location,user_bio,CONCAT('https://twitter.com/',user_screen_name) AS user_twitter_page  FROM $table";
      else
      ...
      $use_repl = false;
      if ($has_repl_col) {
        $check = $link->query("SELECT user_screen_name FROM `$table` WHERE replies > 0 LIMIT 1");
        if ($check && $check->num_rows > 0)
          $use_repl = true;
      }
      if ($use_repl) {
        if ($_GET['export'])
          $qry = "SELECT user_screen_name,user_name AS full_name,replies AS repliers,mentions_of_tweeter,user_verified,user_location,user_bio,CONCAT('https://twitter.com/',user_screen_name) AS user_twitter_page  FROM $table";
        else
          $qry = "SELECT user_screen_name,replies FROM $table";
        $condition = preg_replace("/\s*WHERE/i", " AND ", $condition);
        $condition = "WHERE replies>0 " . $condition;
        $query = "$qry $condition group by user_screen_name order by replies DESC";
      }
      else {
        // Fallback: parse in_reply_to_user_screen_name
        if ($_GET['export'])
          $qry = "SELECT in_reply_to_user_screen_name as user_screen_name, count(tweet_id) as responses_to_tweeter FROM $table ";
        else
          $qry = "SELECT in_reply_to_user_screen_name as user_screen_name, count(tweet_id) as responses_to_tweeter FROM $table ";
        $condition = preg_replace("/\s*WHERE/i", " AND ", $condition);
        $condition = "WHERE in_reply_to_user_screen_name IS NOT NULL AND in_reply_to_user_screen_name <> '' " . $condition;
        $query = "$qry $condition group by in_reply_to_user_screen_name order by responses_to_tweeter DESC";
      }
    }
  }
  elseif ($type == "mention") {
    $title = "Top tweeters (active) with highest number of mentions from others for " . $cases[$table]['name'];
    $subtitle = "Number of times (active) being mentioned by others";
    $yaxis = "mentions";
    $name = "Top mentioned tweeters";

    $has_ment_col = $link->query("SHOW COLUMNS FROM `$table` LIKE 'mentions_of_tweeter'")->num_rows > 0;

    $use_ment = false;
    if ($has_ment_col) {
      $check = $link->query("SELECT user_screen_name FROM `$table` WHERE mentions_of_tweeter > 0 LIMIT 1");
      if ($check && $check->num_rows > 0)
        $use_ment = true;
    }
    if ($use_ment) {
      if ($_GET['export'])
        $qry = "SELECT distinct user_screen_name,mentions_of_tweeter,user_verified,user_location,user_bio,CONCAT('https://twitter.com/',user_screen_name) AS user_twitter_page  FROM $table ";
      else
      ...
      // Fallback: parse user_mentions column from the main table, but only for users WHO HAVE TWEETS
      if ($_GET['export'])
        $qry = "SELECT LOWER(TRIM(REPLACE(REPLACE(SUBSTRING_INDEX(SUBSTRING_INDEX(REPLACE(user_mentions, ',', ' '), ' ', n), ' ', -1), '[', ''), ']', ''))) AS mention_user, COUNT(*) AS mentions_of_tweeter FROM $table CROSS JOIN (SELECT 1 n UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5 UNION SELECT 6 UNION SELECT 7 UNION SELECT 8 UNION SELECT 9 UNION SELECT 10 UNION SELECT 11 UNION SELECT 12 UNION SELECT 13 UNION SELECT 14 UNION SELECT 15 UNION SELECT 16 UNION SELECT 17 UNION SELECT 18 UNION SELECT 19 UNION SELECT 20) AS numbers ";
      else
        $qry = "SELECT LOWER(TRIM(REPLACE(REPLACE(SUBSTRING_INDEX(SUBSTRING_INDEX(REPLACE(user_mentions, ',', ' '), ' ', n), ' ', -1), '[', ''), ']', ''))) AS mention_user, COUNT(*) AS mentions_of_tweeter FROM $table CROSS JOIN (SELECT 1 n UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5 UNION SELECT 6 UNION SELECT 7 UNION SELECT 8 UNION SELECT 9 UNION SELECT 10 UNION SELECT 11 UNION SELECT 12 UNION SELECT 13 UNION SELECT 14 UNION SELECT 15 UNION SELECT 16 UNION SELECT 17 UNION SELECT 18 UNION SELECT 19 UNION SELECT 20) AS numbers ";

      $condition = preg_replace("/\s*WHERE/i", " AND ", $condition);
      $condition = "WHERE user_mentions IS NOT NULL AND user_mentions <> '' AND user_mentions <> '[]' " . $condition;
      // Filter by users WHO HAVE TWEETS in this table for "Active" mentions
      $query = "$qry $condition AND n <= 1 + (LENGTH(REPLACE(user_mentions, ',', ' ')) - LENGTH(REPLACE(REPLACE(user_mentions, ',', ' '), ' ', ''))) GROUP BY mention_user HAVING mention_user IN (SELECT DISTINCT LOWER(user_screen_name) FROM $table) ORDER BY mentions_of_tweeter DESC";
    }
  }
  elseif ($type == "all_mentions") {
    $title = "Top tweeters (all) with highest number of mentions from others for " . $cases[$table]['name'];
    $subtitle = "Number of times (all) being mentioned by others";
    $yaxis = "mentions";
    $name = "Top mentioned tweeters";

    // Check if user_all_mentions table exists
    $has_all_ment_table = $link->query("SHOW TABLES LIKE 'user_all_mentions_$table'")->num_rows > 0;
    $use_all_ment = false;
    if ($has_all_ment_table) {
      $check = $link->query("SELECT user_screen_name FROM user_all_mentions_$table WHERE mentions_of_tweeter > 0 LIMIT 1");
      if ($check && $check->num_rows > 0)
        $use_all_ment = true;
    }
    ...
    if ($use_all_ment) {
      if ($_GET['export'])
        $qry = "SELECT user_all_mentions_" . $table . ".user_screen_name,user_all_mentions_" . $table . ".user_name AS full_name,user_all_mentions_" . $table . ".mentions_of_tweeter,user_all_mentions_" . $table . ".user_verified,user_all_mentions_" . $table . ".user_location,user_all_mentions_" . $table . ".user_bio,CONCAT('https://twitter.com/',user_all_mentions_" . $table . ".user_screen_name) AS user_twitter_page  FROM user_all_mentions_" . $table . " ";
      else
      ...
      // Fallback: parse user_mentions column for EVERYONE mentioned
      if ($_GET['export'])
        $qry = "SELECT LOWER(TRIM(REPLACE(REPLACE(SUBSTRING_INDEX(SUBSTRING_INDEX(REPLACE(user_mentions, ',', ' '), ' ', n), ' ', -1), '[', ''), ']', ''))) AS user_screen_name, COUNT(*) AS mentions_of_tweeter FROM $table CROSS JOIN (SELECT 1 n UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5 UNION SELECT 6 UNION SELECT 7 UNION SELECT 8 UNION SELECT 9 UNION SELECT 10 UNION SELECT 11 UNION SELECT 12 UNION SELECT 13 UNION SELECT 14 UNION SELECT 15 UNION SELECT 16 UNION SELECT 17 UNION SELECT 18 UNION SELECT 19 UNION SELECT 20) AS numbers ";
      else
        $qry = "SELECT LOWER(TRIM(REPLACE(REPLACE(SUBSTRING_INDEX(SUBSTRING_INDEX(REPLACE(user_mentions, ',', ' '), ' ', n), ' ', -1), '[', ''), ']', ''))) AS user_screen_name, COUNT(*) AS mentions_of_tweeter FROM $table CROSS JOIN (SELECT 1 n UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5 UNION SELECT 6 UNION SELECT 7 UNION SELECT 8 UNION SELECT 9 UNION SELECT 10 UNION SELECT 11 UNION SELECT 12 UNION SELECT 13 UNION SELECT 14 UNION SELECT 15 UNION SELECT 16 UNION SELECT 17 UNION SELECT 18 UNION SELECT 19 UNION SELECT 20) AS numbers ";

      $condition = preg_replace("/\s*WHERE/i", " AND ", $condition);
      $condition = "WHERE user_mentions IS NOT NULL AND user_mentions <> '' AND user_mentions <> '[]' " . $condition;
      $query = "$qry $condition AND n <= 1 + (LENGTH(REPLACE(user_mentions, ',', ' ')) - LENGTH(REPLACE(REPLACE(user_mentions, ',', ' '), ' ', ''))) GROUP BY user_screen_name ORDER BY mentions_of_tweeter DESC";
    }
  }
  elseif ($type == "quotes") {
    $title = "Top quoted tweeters for " . $cases[$table]['name'];
    $subtitle = "Number of times being quoted by others";
    $yaxis = "quotes";
    $name = "Top quoted tweeters";
    if ($_GET['export'])
      $qry = "SELECT k2.user_screen_name,k2.user_name AS full_name,count(k1.tweet_id) AS quoted_tweets,k2.user_verified,k2.user_location,k2.user_bio,CONCAT('https://twitter.com/',k2.user_screen_name) AS user_twitter_page FROM $table k1 inner join $table k2 on k2.tweet_id=k1.quoted_tweet_id ";
    else
      $qry = "SELECT k2.user_screen_name,count(k1.tweet_id) AS quoted_tweets FROM $table k1 inner join $table k2 on k2.tweet_id=k1.quoted_tweet_id ";

    // Robust self-join check
    $condition = preg_replace("/\s*WHERE/i", " AND ", $condition);
    $condition = "WHERE k1.quoted_tweet_id is not null AND k1.quoted_tweet_id <> 0 " . $condition;
    $query = "$qry $condition group by k2.user_screen_name order by count(k1.tweet_id) desc";
  }
  elseif ($type == "tweets") {
  ...
  if ($limit > sizeof($user_names))
    $limit = sizeof($user_names);
  ...
  if ($show_query || $show_period) {
    echo "<table style='font-size:10pt; width:600px; background-color:#FFFFFF; border:1px; margin-left:30px'>";
    if ($show_query) {
      echo "<tr><td><b>Search query:</b></td><td> " . $cases[$table]['query'] . "</td></tr>";
    }
    if ($show_period) {
      echo "<tr><td><b>Period:</b> </td><td> From: " . $cases[$table]['from'] . "<br>To: " . $cases[$table]['to'] . "</td></tr>";
    }
    echo "</table><br>";
  }
